...
export const ProjectDetailData: ProjectDetail[] = [
  ...
  },
  {
    ...
    reflection: [
      `사용자 인터랙션부터 데이터 저장과 동기화까지 서비스 전체 흐름을 직접 설계해본 프로젝트였습니다.
      단순히 UI를 구현하는 것을 넘어서, Firebase와 연동한 실시간 데이터 관리, 권한 기반 편집 기능, 클라이언트 요금 최적화까지 고려하며 실제 서비스 구축 방식에 가까운 개발을 경험했습니다.
      컴포넌트 추상화, 서비스 로직 분리, 타입 안전성을 확보하는 구조로 리팩토링하며 코드 품질과 유지보수성에 대한 감각도 키울 수 있었습니다.
      Firebase 리스너 관리, 비동기 예외 처리, 접근성 개선 등 실무에서 마주할 수 있는 고민들을 해결해가며 프론트엔드 개발자로서의 사고 방식과 문제 해결 능력을 한층 더 키울 수 있었습니다.`
    ]
  },
  {
    id: `movieApp`,
    ...
    reflection: [
      `퍼블리셔에서 프론트엔드로 역할을 확장하며, React와 TypeScript로 데이터 흐름과 구조 설계를 직접 다룬 첫 프로젝트입니다.
        API 통신 중 useParams의 타입 추론 문제나 비동기 데이터 처리에 대한 고민이 있었지만, 직접 해결하며 React의 데이터 흐름과 상태 관리에 대한 이해를 할 수 있었습니다.
        SVG 컴포넌트화, 웹폰트 최적화 등 실무 환경에서 흔히 마주할 수 있는 문제를 해결하며 실전 감각을 키웠고,
        프로젝트를 통해 배운 기술들을 더 심화해 전역 상태 관리, 테스트 코드 작성 등으로 확장할 계획입니다.`
    ]
  },
  {
    id: `hcgHr`,
